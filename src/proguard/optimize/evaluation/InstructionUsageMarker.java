...
//            method.getName(clazz).equals("abc");
        ...
        {
            Logger.err_println("Unexpected error while marking instruction usage after partial evaluation:");
    ...
    public void visitCodeAttribute0(Clazz clazz, Method method, CodeAttribute codeAttribute)
    {
        ...
        {
            Logger.out_println();
            Logger.out_println("InstructionUsageMarker ["+clazz.getName()+"."+method.getName(clazz)+method.getDescriptor(clazz)+"]");
        }
        ...
        if (DEBUG) Logger.out_println("Invocation simplification:");
        ...
        if (DEBUG) Logger.out_println("Usage initialization: ");
        ...
        // been been marked.
        if (DEBUG) Logger.out_println("Usage marking:");
        ...
        while (maxMarkedOffset >= 0)
        {
            ...
            if (DEBUG)
            {
                ...
                {
                    Logger.out_println(" -> "+maxMarkedOffset);
                }
            }
        }
        if (DEBUG) Logger.out_println();
        ...
        if (DEBUG) Logger.out_println("Initialization marking: ");
        ...
        // consistent.
        if (DEBUG) Logger.out_println("Extra pop marking:");
        ...
        if (DEBUG_RESULTS)
        {
            Logger.out_println("Instruction usage results:");
            ...
            do
            {
                Instruction instruction = InstructionFactory.create(codeAttribute.code,
                                                                    offset);
                Logger.out_println((isInstructionNecessary(offset)             ? " + " :
                ...
                case InstructionConstants.OP_INVOKEINTERFACE:
                {
                    ...
                    for (int index = 0; index < parameterSize; index++)
                    {
                            ...
                            (usedParameters & (1L << index)) == 0L)
                        {
                            ...
                            if (DEBUG)
                            {
                                Logger.out_println("  ["+offset+"] Ignoring parameter #"+index+" (stack entry #"+stackIndex+" ["+stack.getBottom(stackIndex)+"])");
                                Logger.out_println("    Full stack: "+stack);
                            }
                  ...
                  ParameterVisitor
    {
        ...
        public void visitBranchInstruction(Clazz clazz, Method method, CodeAttribute codeAttribute, int offset, BranchInstruction branchInstruction)
        {
            ...
            {
                if (DEBUG) Logger.out_print("(infinite loop)");
                markInstruction(offset);
            }
            else
            {
...
//            {
//                Logger.out_println("InstructionUsageMarker$MyInitialUsageMarker.visitAnyMethodrefConstant [" + refConstant.getClassName(clazz) + "." + refConstant.getName(clazz) +
...
//            }
                ...
                !NoExternalSideEffectMethodMarker.hasNoExternalSideEffects(referencedMethod))
            {
                ...
                SideEffectClassChecker.mayHaveSideEffects(clazz,
                                                          ...
                                                          referencedMethod))
            {
//                Logger.out_println("  -> mark ["+referencingOffset+"]");
                // Mark the invocation.
                markInstruction(referencingOffset);
            }
            else
            {
                if (DEBUG)
                {
                    Logger.out_println("  ["+referencingOffset+"] Checking parameters of ["+refConstant.getClassName(clazz)+"."+refConstant.getName(clazz)+refConstant.getType(clazz)+"] (pop count = "+referencingPopCount+")");
                }
        ...
        public void visitParameter(Clazz clazz, Member member, int parameterIndex, int parameterCount, int parameterOffset, int parameterSize, String parameterType, Clazz referencedClass)
        {
            ...
            if (DEBUG)
            {
                Logger.out_println("    P"+parameterIndex+
...
//            Logger.out_println("     ["+offset+"] s"+stackEntryIndex+": ["+stackEntry+"]");

            if (stackEntry.computationalType() == Value.TYPE_REFERENCE)
            {
                ReferenceValue referenceValue = stackEntry.referenceValue();
//                Logger.out_println("EvaluationShrinker$MyInitialUsageMarker.createReverseDependencies: ["+offset+"] ["+referenceValue+"]?");
                ...
                if (referenceValue.isNull() != Value.ALWAYS)
                {
                    ...
                    {
//                        Logger.out_println("InstructionUsageMarker$MyInitialUsageMarker.createReverseDependencies: not a TracedReferenceValue");
                        markInstruction(offset);
                    }
                }
            }
                                               ...
                                               InstructionOffsetValue producerOffsets)
        {
            ...
            for (int offsetIndex = 0; offsetIndex < offsetCount; offsetIndex++)
            {
                if (producerOffsets.isNewinstance(offsetIndex))
                {
                    ...
                    {
                        if (DEBUG) Logger.out_println("  Inserting reverse dependency from instance producers ["+producerOffset+"] to ["+offset+"]");
                                               ...
                                               InstructionOffsetValue visitedOffsets)
    {
        ...
        if (!isInstructionNecessary(producerOffset))
        {
            ...
            {
                // Mark the producer.
                if (DEBUG) Logger.out_print("  Marking initialization of v"+variableIndex+" at ");
                                      ...
                                      int branchTarget)
    {
        ...
        {
            if (DEBUG) Logger.out_print("["+branchOrigin+"->"+branchTarget+"]");
                                     ...
                                     int stackIndex)
    {
        ...
        {
            if (DEBUG) Logger.out_print("["+instructionOffset+".s"+stackIndex+"],");
    ...
    private void markInstruction(int instructionOffset)
    {
        ...
        {
            if (DEBUG) Logger.out_print(instructionOffset+",");
