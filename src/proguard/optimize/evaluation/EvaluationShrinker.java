...
//            method.getName(clazz).equals("abc");
        ...
        {
            Logger.err_println("Unexpected error while shrinking instructions after partial evaluation:");
            ...
            ex.printStackTrace();
            Logger.err_println("Not optimizing this method");
    ...
    public void visitCodeAttribute0(Clazz clazz, Method method, CodeAttribute codeAttribute)
    {
        if (DEBUG_RESULTS)
        {
            Logger.out_println("EvaluationShrinker ["+clazz.getName()+"."+method.getName(clazz)+method.getDescriptor(clazz)+"]");
        }
        ...
        if (DEBUG) Logger.out_println("Static invocation fixing:");
        ...
        if (DEBUG) Logger.out_println("Backward branch fixing:");
        ...
        // The virtual machine's verification step is not smart enough to see
        // the code isn't reachable, and may complain otherwise.
        if (DEBUG) Logger.out_println("Non-returning subroutine fixing:");
        ...
        if (DEBUG) Logger.out_println("Stack consistency fixing:");
        ...
        if (DEBUG) Logger.out_println("Deleting unused instructions");
        ...
        if (DEBUG_RESULTS)
        {
            Logger.out_println("Simplification results:");
            ...
            do
            {
                Instruction instruction = InstructionFactory.create(codeAttribute.code,
                                                                    offset);
                Logger.out_println((instructionUsageMarker.isInstructionNecessary(offset)             ? " + " :
    ...
    implements    InstructionVisitor
    {
        ...
        public void visitAnyInstruction(Clazz clazz, Method method, CodeAttribute codeAttribute, int offset, Instruction instruction)
        {
                                                          ...
                                                          offset))
            {
                ...
                if (DEBUG) Logger.out_println("  Setting infinite loop instead of "+instruction.toString(offset));
            }
        }
        ...
        public void visitBranchInstruction(Clazz clazz, Method method, CodeAttribute codeAttribute, int offset, BranchInstruction branchInstruction)
        {
            ...
            if (branchInstruction.canonicalOpcode() == InstructionConstants.OP_JSR)
            {
                ...
                if (!instructionUsageMarker.isInstructionNecessary(nextOffset))
                {
                    ...
                    if (DEBUG) Logger.out_println("  Adding infinite loop at ["+nextOffset+"] after "+branchInstruction.toString(offset));
                }
            }
        }
    }
        ...
        public void visitAnyInstruction(Clazz clazz, Method method, CodeAttribute codeAttribute, int offset, Instruction instruction)
        {
            ...
            if (instructionUsageMarker.isInstructionNecessary(offset))
            {
                ...
                if (popCount > 0)
                {
                    ...
                    {
                        if (DEBUG) Logger.out_println("  Inserting before marked consumer "+instruction.toString(offset));
                ...
                if (pushCount > 0)
                {
                    ...
                    if (requiredPopCount > 0)
                    {
                        if (DEBUG) Logger.out_println("  Inserting after marked producer "+instruction.toString(offset));
            ...
            else if (instructionUsageMarker.isExtraPushPopInstructionNecessary(offset))
            {
                ...
                if (popCount > 0)
                {
                    ...
                    {
                        if (DEBUG) Logger.out_println("  Replacing unmarked consumer "+instruction.toString(offset));
                ...
                if (pushCount > 0)
                {
                    ...
                    {
                        if (DEBUG) Logger.out_println("  Replacing unmarked producer "+instruction.toString(offset));
                ...
                isDupOrSwap(simpleInstruction))
            {
                ...
                if ((newOpcodes & ~0xff) == 0)
                {
                    ...
                    if      (newOpcode == InstructionConstants.OP_NOP)
                    {
                        ...
                        if (DEBUG) Logger.out_println("  Deleting marked instruction "+simpleInstruction.toString(offset));
                    }
                    else if (newOpcode == oldOpcode)
                    {
                        ...
                        if (DEBUG) Logger.out_println("  Marking unchanged instruction "+simpleInstruction.toString(offset));
                    }
                    else
                    {
                        ...
                        if (DEBUG) Logger.out_println("  Replacing instruction "+simpleInstruction.toString(offset)+" by "+replacementInstruction.toString());
                    }
                }
                else
                {
                    ...
                    if (DEBUG) Logger.out_println("  Replacing instruction "+simpleInstruction.toString(offset)+" by");
                    int count = 0;
                    while (newOpcodes != 0)
                    {
                        ...
                        if (DEBUG) Logger.out_println("    "+replacementInstruction.toString());
                        newOpcodes >>>= 8;
                    }
        ...
        public void visitBranchInstruction(Clazz clazz, Method method, CodeAttribute codeAttribute, int offset, BranchInstruction branchInstruction)
        {
            if (instructionUsageMarker.isInstructionNecessary(offset))
            {
                    ...
                    !instructionUsageMarker.isStackEntryPresentBefore(offset, instructionUsageMarker.getStackBefore(offset).size() - 1))
                {
                    ...
                    if (DEBUG) Logger.out_println("  Replacing branch instruction "+branchInstruction.toString(offset)+" by "+replacementInstruction.toString());
                }
            }
            else
            {
        ...
        public void visitAnySwitchInstruction(Clazz clazz, Method method, CodeAttribute codeAttribute, int offset, SwitchInstruction switchInstruction)
        {
            if (instructionUsageMarker.isInstructionNecessary(offset))
            {
                    ...
                    !instructionUsageMarker.isStackEntryPresentBefore(offset, instructionUsageMarker.getStackBefore(offset).size() - 1))
                {
                    ...
                    if (DEBUG) Logger.out_println("  Replacing switch instruction "+switchInstruction.toString(offset)+" by "+replacementInstruction.toString());
                }
            }
            else
            {
                                        ...
                                        int     computationalType)
    {
        ...
        if (DEBUG) Logger.out_println(": "+replacementInstruction.toString(offset));
                                           ...
                                           ConstantInstruction constantInstruction)
    {
        ...
        if (DEBUG) Logger.out_println("  Replacing by static invocation "+constantInstruction.toString(offset)+" -> "+replacementInstruction.toString());
    ...
    private void replaceByInfiniteLoop(Clazz clazz,
                                       int   offset)
    {
        if (DEBUG) Logger.out_println("  Inserting infinite loop at ["+offset+"]");
